function [F, eta, gamma, Plot_signal] = FLIPS_Solver(X,phi,epsilon,maxiter,oracle,betainv,momentum_para)
...
for t = 1:maxiter
    ...
    for l = 1:N
        ...
        % grad_check1 = -grad_dummy'*phiadjX(:,l) 

        %% Descent direction oracles

        if strcmp(oracle, 'SimpleQO')
            % simple quadratic oracle
            g = Simple_quad_oracle(H(:,l), grad_dummy, betainv,tau) ;
        elseif strcmp(oracle, 'AcceleratedQO')
            % accelerated quadratic oracle 
            d_old = D_old(:,l) ;
            [g, D_old(:,l)] = Accelerated_quad_oracle(H(:,l), grad_dummy, d_old, betainv, momentum_para,tau) ;
        end
...
%% only to check the maximum stepsize

% c_max_step = normx_minus_eps(l)*norm_phiH_sq(l) - ip_xphiH(l)^2 ;
% gamma_max = max( (-b + sqrt(b^2 - 4*a*c_max_step))/(2*a) , (-b - sqrt(b^2 - 4*a*c_max_step))/(2*a)  ) ;
%%
