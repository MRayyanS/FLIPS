function [F,time_count,avg_iter_count] = FLIPS_CPU_time(X,dictionary,epsilon,maxiter,oracle,betainv,momentum_para,patch_dimensions,F_star,conv_threshold)
...
%% Update
        ...
        f = eta_val*h ;
        convergence_check = norm(f - f_star,2)/norm_f_star ;
    ...
    avg_iter_count  =  avg_iter_count + (1/l)*(t - avg_iter_count) ;

    F(:,l)     = ( (phiadj_x'*h)/(norm(h,2))^2 ).*h ;
